...
const Shop = () => {
    ...
    return (
        <div className="flex flex-col gap-5 p-8 bg-slate-300 h-screen">
            <h1 className="text-2xl font-bold">Shop</h1>
            <ul className="grid gap-4 grid-cols-3 grid-rows-3 justify-items-center">
                {items.map((item) => {
                    return (
                        <li key={item.items[0].id}>
                            <Link className="flex flex-col justify-center items-center" to={`/shop/${item.items[0].id}`} state={{ shopItem: {...item.items[0]} }}>
